...
def CCI(CLOSE,HIGH,LOW,N=14):
    ...
    result=(TYP-MyTT.MA(TYP,N))*1000/(15*MyTT.AVEDEV(TYP,N))
    return result
def KDJ(CLOSE,HIGH,LOW, N=9,M1=3,M2=3):
    ...
    输出J:3*K-2*D
    '''
    RSV=(CLOSE-MyTT.LLV(LOW,N))/(MyTT.HHV(HIGH,N)-MyTT.LLV(LOW,N))*100
    K=MyTT.SMA(RSV,M1,1)
    D=MyTT.SMA(K,M2,1)
    J=3*K-2*D
    return K,D,J
def MFI(CLOSE,HIGH,LOW,VOL,N=14):
    ...
    '''
    TYP = (HIGH + LOW + CLOSE)/3
    V1=MyTT.SUM(MyTT.IF(TYP>MyTT.REF(TYP,1),TYP*VOL,0),N)/MyTT.SUM(MyTT.IF(TYP<MyTT.REF(TYP,1),TYP*VOL,0),N)  
    return 100-(100/(1+V1))  
