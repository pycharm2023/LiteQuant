...
class XtQuantTrader(object):
    ...
    #########################
        ...
        def on_push_OrderStockAsyncResponse(seq, resp):
            ...
            if callback:
                ...
                callback(resp)
                e = self.queuing_order_errors.pop(seq, None)
                if e is not None:
                    self.callback.on_order_error(e)
                    self.handled_async_order_stock_order_id.discard(seq)
            return
        ...
        def on_push_OrderError(data):
            if data.seq in self.handled_async_order_stock_order_id or data.seq in self.sync_order_stock_order_id:
                self.handled_async_order_stock_order_id.discard(data.seq)
                self.sync_order_stock_order_id.discard(data.seq)
                self.callback.on_order_error(data)
            else:
                self.queuing_order_errors[data.seq] = data
    ...
    def order_stock_async(self, account, stock_code, order_type, order_volume, price_type, price, strategy_name='',
        ...
        self.handled_async_order_stock_order_id.add(seq)
        return seq
